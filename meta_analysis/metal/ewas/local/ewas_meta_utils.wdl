...
task run_metal {
  ...
  parameter_meta  {
    ...
    ewas_files: "Array of input EWAS summary stats for the specified chromosome."
  }
...
task merge_results {
  ...
  command <<<
    ...
    results_string="${sep=" " ewas_results}"
  ...
  parameter_meta {
    ewas_results: "Array of results split by chromosome."
  }
  
  meta {
    description: "Merge the chromosome results back together so they can be input into the ewas plotting workflow."
    author: "Jesse Marks"
    email: "jmarks@rti.org"
  }
}
...
task top_results {
  ...
  command <<<
    ...
    awk -v pval="${pvalue}" '
      ...
      NR>1 {
        if ($F["P-value"] <= pval)
          print $0;
      }' <(zcat ${ewas_results}) > "final_results_p_lte_${pvalue}.tsv"
  ...
  parameter_meta {
    ewas_results: "The EWAS meta-analysis results containing only the chromosomes of interest."  

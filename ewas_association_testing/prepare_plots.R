...
option_list = list(
  ...
  make_option(c("-f", "--fdr"), default=0.05, type='double',
              help="False discovery rate (Benjamini-Hochberg method). [default %default]"),
...
manhattan.plot(chr=resu_annot_sub$chr_ordered,
               pos=resu_annot_sub$pos,
               pvalue=resu_annot_sub$P_VAL,
               col=man_colors,
               sig.level=bonferroni,
               sig.level2=fdr_value)

dev.off()


#Save analysis sample sizes, lambda, and model number
#Model <- opt$m
#TotalSampleSize <- opt$s
#N_EverCannabis <- opt$ncases
#tab <- cbind(Model, TotalSampleSize, N_EverCannabis, lambda)

basename <- paste0(opt$o, "_qq.png")
png(basename, width=950, height=500)

# lambda is a measure of inflated p-values. Should be ~1
lambda <- qchisq(p=median(resu_annot_sub$P_VAL,  na.rm=T),  df = 1, lower.tail = F) /
          qchisq(p=0.5, df=1) #  gives the quantile function

qqunif.plot(resu_annot_sub$P_VAL,

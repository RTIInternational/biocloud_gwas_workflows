# GenomicSEM User Model GWAS Workflow

## Overview

Runs a per-SNP GWAS under a user-specified GenomicSEM structural equation model (SEM), defined via a lavaan model syntax file. The workflow:
...
[gsem_user_model_gwas.wdl](gsem_user_model_gwas.wdl)
...
| `model_lavaan` | `File` | Lavaan model syntax file for `userGWAS()`. |
...
| --- | --- | --- | --- |
| `estimation_method` | `String` | `"DWLS"` | Estimation method for `userGWAS()`. |
...
| `not_printwarn` | `Boolean` | `false` | If true, suppress per-SNP lavaan warnings/errors in userGWAS output. |
| `sub` | `Array[String]?` | — | Optional subset of model components to return from userGWAS. |
| `toler` | `Float?` | — | Optional matrix inversion tolerance for userGWAS. |
| `snpse` | `Float?` | — | Optional SNP standard error override for userGWAS. |
| `gc` | `String` | `"standard"` | Genomic control mode: `standard`, `conserv`, or `none`. |
| `mpi` | `Boolean` | `false` | Enable MPI/multi-node mode for userGWAS. |
| `smooth_check` | `Boolean` | `false` | Enable smoothing diagnostics in userGWAS. |
| `twas` | `Boolean` | `false` | Enable TWAS mode in userGWAS. |
...
| `usergwas_output_prefix` | `String` | `"usergwas/gsem_usergwas_output"` | Output prefix for user-model GWAS output. |
...
For each of `munge`, `ldsc`, `sumstats`, and `usergwas`, memory is automatically calculated as **1.5x the size of the task's inputs**, rounded up to `(2^n - 1)` GB, unless the corresponding `*_mem_gb` override is provided:
...
| sumstats | Preprocessed sumstats file sizes + `ref_snp_list` size |
| usergwas | `ldsc_rds` + `sumstats_rds` + `model_lavaan` size |
...
See [gsem_user_model_gwas_inputs_min.json](gsem_user_model_gwas_inputs_min.json) for a minimal example and [gsem_user_model_gwas_inputs_full.json](gsem_user_model_gwas_inputs_full.json) for a full example with all optional parameters set.
...
docker run -ti \
    ...
    -e main="$BIOCLOUD_GWAS_WORKFLOWS_DIR/association_testing/genomic_sem/$VERSION/user_model_gwas/gsem_user_model_gwas.wdl" \
    -e name="gsem_user_model_gwas" \
    -e description="GenomicSEM user model GWAS workflow" \
    -e readme="$BIOCLOUD_GWAS_WORKFLOWS_DIR/association_testing/genomic_sem/$VERSION/user_model_gwas/README.md" \
...
docker run -ti \
    ...
    -e repo_dir="$BIOCLOUD_GWAS_WORKFLOWS_DIR" \
    -e main="$BIOCLOUD_GWAS_WORKFLOWS_DIR/association_testing/genomic_sem/$VERSION/user_model_gwas/gsem_user_model_gwas.wdl" \
    -e name="gsem_user_model_gwas_$VERSION" \
    -e description="GenomicSEM user model GWAS workflow" \
    -e readme="$BIOCLOUD_GWAS_WORKFLOWS_DIR/association_testing/genomic_sem/$VERSION/user_model_gwas/README.md" \
    --rm $HEALTHOMICS_TOOLS_DOCKER_IMAGE
```

...
workflow genesis_gwas_wf{
    ...
    # CPU/Memory each split of genesis will consume
    # Keep this VERY SMALL (1CPU/2GB) if you're splitting within chr because genesis is fast and you might have hundreds or thousands of splits
    Int genesis_cpu_per_split = 1
    Int genesis_mem_gb_per_split = 2
    ...
    scatter(chr_index in range(length(genos_in))){

        String sCovars = 
        call GENESIS_CHR.genesis_gwas_chr_wf as genesis{
            input:
                fileInGeno = genos_in[chr_index],
                fileInPheno = pheno_file,
                outBaseName = study_output_basename,
        ...
        #         output_filename = basename(genesis.summary_stats, ".tsv.gz") + ".good_ids.tsv.gz",
        #         rescue_rsids = rescue_rsids,
        #         output_compression = "gzip",
        #         cpu = genesis_cpu_per_split,
        #         mem_gb = genesis_mem_gb_per_split
        # }
    ...
    #         output_dir_name = study_output_basename + "_genesis_chr_output"
    # }
    ...
    #         output_filename = study_output_basename + ".genesis.MetaAssoc.tsv"
    # }
